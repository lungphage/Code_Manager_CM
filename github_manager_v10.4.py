...
# -*- coding: utf-8 -*-
"""
GitHub/Gitee Manager v10.4 - 本地代码托管管理工具
Author: LZF
Version: 10.4.0
"""
...
class GitOps:
    ...
    def pull_unrelated(self, path, remote="origin", branch="main"):
    ...
    def pull_with_strategy(self, path, remote="origin", branch="main", strategy=None):
        """拉取远程，可指定合并策略（如 ours）。"""
...
class App:
    ...
    def _tab_about(self):
        ...
        ttk.Label(f, text="v10.4 优化内容",
                  font=("Segoe UI", 11, "bold"),
                  foreground=t["accent"]).grid(row=6, column=0, columnspan=2, pady=(4, 6))

        features = [
            "• [性能] GradientButton 消除冗余渐变绘制，新增颜色缓存",
            "• [修复] NetworkManager 合并重试异常，修复3次限流后返回None",
            "• [修复] PlatformAPI 补全 username 属性，修复Fork克隆AttributeError",
            "• [修复] list_repos 缓存 r.json()，消除每页3次重复解析",
            "• [封装] GitOps 新增 pull_unrelated/pull_with_strategy 方法",
            "• [重构] App.log 与 _log_to_ui 合并，线程安全逻辑统一",
            "• [健壮] _get_theme 增加 _normalize_theme 统一补全缺失主题key",
            "• [安全] Token 本地加密存储（机器绑定XOR+base64），重启免重输",
            "• [线程] _do_upload/commit/push 等控件值读取移至主线程",
            "• [线程] _load_issues/_load_pulls 状态读取移至主线程",
        ]
