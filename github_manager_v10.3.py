...
# -*- coding: utf-8 -*-
"""
GitHub/Gitee Manager v10.3 - 本地代码托管管理工具
Author: LZF
Version: 10.3.0
"""
...
class App:
    ...
    def _tab_about(self):
        ...
        ttk.Label(f, text="v10.3 修复内容",
                  font=("Segoe UI", 11, "bold"),
                  foreground=t["accent"]).grid(row=6, column=0, columnspan=2, pady=(4, 6))

        features = [
            "• [修复] 批量上传：替换不存在的 askdirectories API",
            "• [修复] 线程安全：分支列表/Fork列表更新改为 root.after()",
            "• [修复] 设置页：保存 Combobox 引用，修复 StringVar.config() 崩溃",
            "• [修复] 提交推送：消除双重 add+commit，去掉静默强制推送",
            "• [修复] 日志轮转：修正文件重命名顺序，正确生成 .1/.2... 备份",
            "• [安全] 导出配置时自动剥离 Token 字段",
            "• [安全] 程序退出时将 Token 删除持久化到磁盘",
            "• [安全] 推送失败不再静默强制推送，改为提示用户手动确认",
            "• [质量] import re 移至模块顶层",
            "• [质量] 裸 except 改为具体异常类型",
        ]
    ...
    def _do_commit_push(self):
        ...
        def task():
            try:
                ...
                self.log("正在同步远程仓库...")
                pull_ok, _, _ = self.git._run(
                    ["pull", "origin", current_branch, "--allow-unrelated-histories", "--no-edit"],
                    cwd=path
                )
                if not pull_ok:
                    self.log("拉取失败，尝试以本地为准合并...")
                    self.git._run(
                        ["pull", "origin", current_branch, "--allow-unrelated-histories", "--no-edit", "-X", "ours"],
                        cwd=path
                    )
